...
portrait_wallpapers_folder = Path("/home/fed/Pictures/wallpapers/portrait")

landscape_images = [f for f in landscape_wallpapers_folder.glob("*") if f.suffix.lower() in {'.jpg','.jpeg','.png','.gif','.bmp','.webp'}]
random_landscape_img = random.choice(landscape_images)
portrait_images = [f for f in portrait_wallpapers_folder.glob("*") if f.suffix.lower() in {'.jpg','.jpeg','.png','.gif','.bmp','.webp'}]
